...
$bountyCommands += 'cl /nologo /std:c++20 /EHsc /W4 /WX /MT /Od /Fo"{0}" /Fe"{1}" "{2}"' -f `
    (Join-Path $bountyOutput 'routine_plan_tests.obj'), (Join-Path $bountyOutput 'routine_plan_tests.exe'), `
    (Join-Path $PSScriptRoot 'routine_plan_tests.cpp')
$bountyCommands += 'if errorlevel 1 exit /b 1'
$bountyCommands += '"' + (Join-Path $bountyOutput 'routine_plan_tests.exe') + '"'
$bountyCommands += 'if errorlevel 1 exit /b 1'
$bountyCommands += 'exit /b 0'
$bountyCommandFile = Join-Path $bountyOutput 'run-routine-plan-tests.cmd'
[IO.File]::WriteAllLines($bountyCommandFile, $bountyCommands, [Text.Encoding]::Default)
& $env:ComSpec /d /c $bountyCommandFile
if ($LASTEXITCODE -ne 0) { throw 'Routine plan regression tests failed.' }
