...
$bountyCommands += 'cl /nologo /std:c++20 /EHsc /W4 /WX /MT /Od /Fo"{0}" /Fe"{1}" "{2}"' -f `
    (Join-Path $bountyOutput 'routine_logic_tests.obj'), (Join-Path $bountyOutput 'routine_logic_tests.exe'), `
    (Join-Path $PSScriptRoot 'routine_logic_tests.cpp')
$bountyCommands += 'if errorlevel 1 exit /b 1'
$bountyCommands += '"' + (Join-Path $bountyOutput 'routine_logic_tests.exe') + '"'
$bountyCommands += 'if errorlevel 1 exit /b 1'
$bountyCommands += 'exit /b 0'
$bountyCommandFile = Join-Path $bountyOutput 'run-routine-logic-tests.cmd'
[IO.File]::WriteAllLines($bountyCommandFile, $bountyCommands, [Text.Encoding]::Default)
& $env:ComSpec /d /c $bountyCommandFile
if ($LASTEXITCODE -ne 0) { throw 'Routine logic regression tests failed.' }
