...
Targets follow daily walking routes in Rhodes, Blackwater, Valentine, Strawberry, Saint Denis, Van Horn and Annesburg. Each contract keeps stable workplace, lunch, errands, evening and nighttime destinations. Work is **06:00–11:00 and 12:00–16:00**, lunch **11:00–12:00**, errands **16:00–19:00**, evening **19:00–03:00**, and rest **03:00–06:00**. Old per-target clock offsets do not shift these windows. The target returns to the same workplace after lunch. Cards describe intended places and times; the debug panel distinguishes intentions from observed behavior.

At an accepted area the target attempts a compatible existing world scenario: work, eating/drinking, public ambient behavior, social/campfire activity or rest/sleep. Discovery checks point type, existence, activity, occupancy, ped compatibility, loaded navigation/collision and usable interiors. No beds or in-place animations are invented. Strawberry and Annesburg have no verified usable local saloon in the reviewed sources; their lunch plans use explicitly identified public meal fallbacks. A visit never proves the target ate or slept. Shopping transactions, haircuts, minigames, theatre attendance and tram travel remain disabled.

Targets walk between stops and allow normal scenario exits before travel, with bounded recovery. Successful activities stay running until the schedule, validity or encounter priority changes. Failed points receive cooldowns, and unavailable activities or destinations retain **45 m ambient wandering** around the last accepted area with retries. The matching search circle updates on arrival. Combat, search, restraints and contract interactions retain priority and ownership; routines resume for the current clock afterward. See [the daily routine implementation and test checklist](docs/daily-routine-system.md). Simulated-native tests cannot prove in-game navigation, furniture alignment, animation or prop quality.
...
**U** starts creation on the next game frame after refreshing the player state. Temporary ped-creation failures retry across frames while the model stays loaded. A returned ped handle is retained while it becomes available instead of being overwritten by another spawn attempt. If the game's NPC pool is full, the request stays pending and retries after three seconds; it never removes unrelated NPCs. Model and portrait failures also retain the request. Details are written on failures to `BountyContracts-startup.log` beside the ASI, including download handle/status/name and free NPC slots; include that file when reporting a recurring failure.

Failed or cancelled targets also remain tracked until a later frame confirms that the original entity has disappeared. A zeroed `DELETE_PED` argument alone is not treated as confirmation. A pending request waits for cleanup before creating another target. Deletion retries only touch the matching model owned by this script. `BountyContracts-cleanup.log` records requested, confirmed, blocked, and released targets with counts, so a failed deletion can be distinguished from general NPC pool pressure. The pool count alone cannot identify which script owns the occupied slots.

Location rejection no longer ends a contract request with a **NO SAFE TARGET LOCATION AVAILABLE** prompt. Preparation tries additional compatible stops and towns in bounded passes, and a retained request retries automatically until a location is ready. The previous hunt survives failed preparation; a post-capture placement rejection cleans up its provisional target before another attempt. A successful request still requires both a target and a portrait before the existing card/handoff flow starts. Outdoor, ground and clearance checks remain in place. Rejected locations remain in `BountyContracts-startup.log` for diagnosis. See [the startup changes and remaining engine limits](docs/routine-radius-and-startup-2026-09-07.md).
...
Targets keep one weapon loadout: setup removes inherited weapons once before choosing fists, knife or revolver. They pursue with the game's combat AI and search the last place they saw you after losing sight for eight seconds. A seated target now receives an animated exit request and ordinary task clear; combat and the first weapon draw wait until the scenario, sitting and exiting flags have cleared. Refused combat/search exits use a normal-exit hint. These paths never use immediate exit hints or immediate task clears. An exit that has not started and a queued combat task get 2.5 seconds to settle; an observed exit animation gets eight seconds. Missing-task recovery still uses a one-second grace and three-second retry limit. After ten seconds of searching, targets resume an available scheduled destination once combat and restraint have ended. They remember you and can re-engage on sight. Recovery does not interrupt ragdoll, getting up, lasso, hogtying or fully hogtied states. Flee bit 32768 follows the paired armed-enemy/flee transition in Flaco's mission; the full evidence and other retained flags are in [the AI audit](docs/target-ai-audit.md).

The owner confirmed that the first seated-target fix produced an attacking knife target, but it popped out of the chair immediately. The deferred exit addresses that abrupt transition; smooth chair animation still needs in-game verification. The inspection collision safeguard did not resolve the displaced card: the owner subsequently reported a downward camera from both entry views while ordinary satchel items looked normal. That safeguard has been rolled back. A bounded, read-only inspection trace now records the actual card attachment, visibility, position and camera state. The owner subsequently reported normal card framing again, without a confirmed cause; the radius/startup changes leave inspection unchanged. See [the chair followup](docs/chair-exit-and-card-inspection-2026-09-07.md) and [the card investigation](docs/card-framing-investigation-2026-09-07.md).

The September 2026 audit also retains one **U** release for up to three seconds during this card's put-away outro, including when its prop retires before the native task. Other blocked interactions do not queue that input; pause, fade, player change, handoff, payment or expiry cancels this short input buffer. Once an eligible press starts preparation, the pending-request rules above govern retries. Pauses inside streaming waits freeze gameplay time while streaming retains its wall-clock timeout. Startup rejects a target that disappears or dies during capture, and gameplay rechecks suspension and target loss after yielding interactions. Wanted tracking continues after a photographed corpse disappears, and trail cleanup unregisters only the contract target. The working capture/card/handoff/payment functions are unchanged. Capture failure after replacement has begun can temporarily leave no active hunt while the pending request retries, and a transient unavailable texture status can still hide the card; the earlier evidence is in [the full audit and test checklist](docs/audit-2026-09-06.md).
...
Run `./tests/run-tests.ps1` from PowerShell to compile and execute twenty-three suites: deterministic AI, handoff, keyboard, card-texture, inspection trace, spawn/input/pause/request retries, contract-start orchestration, portrait-startup, portrait-cache, owned-ped cleanup, diagnostic-driver, native AI bridge, contract lifecycle, routine spawn, routine scheduling logic, routine plans, native routine runtime, routine card drawing, debug display, debug blips, debug observation bridge, search-area tracking, and startup trace file writer. The native-shim tests exercise actual production functions. Source review and simulated native responses establish the code's decisions, but cannot verify in-game ground placement, pathfinding, scenario props, animation, card readability or rendering. Build output stays under `tmp/tests`.

[Town target routines](docs/target-routines.md) explains schedules, fallback, matching search/wander areas, bounded spawn candidates and how to add towns and locations. [Location sources](docs/routine-location-sources.md) records the coordinate evidence and omitted sites. [Activities and transit evidence](docs/routine-activities-and-transit.md) documents smoking/drinking, venue availability, the specific blockers for games/theatre/trams and the remaining in-game checks.
...
For routines, inspect outdoor arrivals in each town, daily and midnight destination changes, unavailable-location fallback, smoking/drinking entry and exit, and resumed travel after escaping or releasing a restrained target. Check the card's six clue lines at your resolution and confirm that fast travel or a time change preserves the same target and habits. Indoor combat remains allowed if the encounter moves there.
