...
The owner confirmed that the first seated-target fix produced an attacking knife target, but it popped out of the chair immediately. The new deferred exit addresses that abrupt transition. The subsequent report of a card held off to the right is addressed with collision prevention for the tracked inspection prop and the same per-frame camera exclusion used by Rockstar document scripts. Smooth chair animation and corrected card framing still need in-game verification; see [the followup and test checklist](docs/chair-exit-and-card-inspection-2026-09-07.md).
