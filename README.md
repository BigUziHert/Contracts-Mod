...
The current `photo-test-v3` repeats that test with scheduled binding. It also logs binding resets, actual named-render-target selection/draw counts, and `photo-timing-v1` records for the initial and second capture. These separate native-call latency from asset/upload/readback waits; the previous failing second capture took about 34 seconds without identifying the slow step. Timing records are also appended to ordinary portrait-startup failures. The optional render-target behavior is unchanged so its use can be observed rather than guessed.

Fully exit and restart the game with the diagnostic ASI, load story mode, stand on foot outside menus, and press **U once**. A portrait card now opens: look at the face, **flip it**, then use **Put Away** within 60 seconds. Wait for **Photo test finished**. Press **U** again and repeat with another NPC; note whether the face changes. This build creates one hidden provisional NPC per run and removes it afterward without issuing a bounty. A clerk request runs the same test; completing a bounty is unnecessary. Read `BountyContracts-photo-test.log` and `BountyContracts-cleanup.log` beside the installed ASI. Restore the normal ASI and restart to resume bounty gameplay.
...
- `card_opened` / `card_put_away` / `card_removed` / `card_inspection`: run the actual inspection, flip-panel rendering, texture binding, and cleanup. Log the accepted card's original handle, ownership, existence, bind count, and item-task state. Before continuing, confirm both the original object disappeared and the inspector's native item task ended. A timeout, failed opening, or unconfirmed retirement stops later probes. For a borrowed fallback prop, removal confirmation covers the accepted prop only, not an earlier failed object attempt inside `OpenCard`.
- `A_reopen_without_write`: after the card retires, release the download and reopen the same slot, without another capture or write.
- `B_second_capture`: only if A succeeds, capture the same subject again and read it back.
- `photo-timing-v1`: release, asset loading, idle wait, generation, write, upload, cleanup, and readback durations. `writeCallMaxMs` and `requestCallMaxMs` measure individual native calls separately from their polling windows. `totalMs` also includes frame yields and the remaining native work, so it need not equal the sum of the listed stages.
