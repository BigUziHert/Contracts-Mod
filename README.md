...
Run `./tests/run-tests.ps1` from PowerShell to compile and execute the deterministic AI, handoff, keyboard, card-texture, spawn, portrait-startup, and portrait-cache tests. The native-shim tests exercise actual production functions; they cannot verify the game's rendering or establish the cause of an in-game engine failure. Build output stays under `tmp/tests`.
