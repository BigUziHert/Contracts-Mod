...
The routine startup fix corrects checks that could reject a valid standing target with **NO SAFE TARGET AVAILABLE**. Saved spawn points are rechecked directly, and placement now allows for the ped's body height and a short physics-settling period before revealing it. Outdoor, ground and clearance checks remain in place. A failed location check adds `routine-start-v2` details to `BountyContracts-startup.log`; the generic **Unknown error FFFFFFFF** dialog has no confirmed cause from the available logs. See [the placement fix report](docs/routine-spawn-fix-2026-09-06.md).
